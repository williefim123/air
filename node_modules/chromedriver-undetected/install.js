...
function random_cdc()
{
  ...
  str = str.split("");
  str[str.length-6] = str[str.length-6].toUpperCase();
  str[str.length-5] = str[str.length-5].toUpperCase();
  str[2] = str[0];
  str[3] = '_';
  str = str.join("");
...
function patchFile(path)
{
  ...
  let hexCdc = '6364635f';
  let newCdc = random_cdc();
  let newCdcHex = Buffer.from(newCdc, 'utf8').toString('hex');
  contents = contents.toString(encoding).replace(/6364635f.{44}/g, newCdcHex); //.replace(/cdc_.{22}/g, newCdc);
  let buf = Buffer.from(contents, encoding);
...
(async function install() {
  try {
    ...
    patchFile(libPath+"/"+chromedriverBinaryFileName);
    fixFilePermissions();
